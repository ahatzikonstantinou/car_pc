...
class RadioSettings( MediaDeviceSettings ):
	...
	def Load( self ):
		if( not os.path.isfile( RadioSettings.Filename ) ):
			return

		with open( RadioSettings.Filename, mode='r' ) as f:
			settingsDict = {}
			dict = json.load( f )
			#pprint.pprint( dict )
	...
	def Save(self):
		with open( RadioSettings.Filename, mode='w' ) as f:
