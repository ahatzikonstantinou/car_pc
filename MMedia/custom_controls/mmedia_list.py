...
class MMediaListItem( MediaTrack ):
	def __init__( self, hash, text, is_enabled=True, is_playlist=False, media_type = MediaType.OTHER ):
...
class MediaImageList( wx.ImageList ):
	def __init__( self, width = 16, height = 16, mask = True, initialCount = 1 ):
		wx.ImageList.__init__( self, 16, 16, mask, initialCount )
		image_dir = os.getcwd() + '/images/' + str(width) +'x' + str(height) + '/'
		images = ['folder.png', 'audio.png', 'video.png', 'image-x-generic.png', 'empty.png']
		for i in images:
			self.Add( wx.Bitmap( image_dir + i ) )
			
	def GetImageIndex( self, media_type ):
		if( media_type == MediaType.DIR ):
			return 0
...
class MMediaList:
	...
	PLAYLIST = 1
	SYSTEM_PLAYLISTS = 2
	def __init__( self, parent_panel, settings, type=PLAYLIST, with_list_title=False, select_list_item_callback = None, click_list_item_callback = None, actions=[], with_images = False ):
		...
		list_style = wx.LC_REPORT | wx.LC_NO_HEADER | ( 0 if type == MMediaList.FILELIST else wx.LC_SINGLE_SEL ) | wx.LC_HRULES | wx.LC_VRULES
		if( self.with_images ):
			list_style = list_style | wx.LC_ICON
			
		self.list = CustomListCtrl( self._panel, style = list_style )
		...
		#if( self.with_images ):
		self.image_list = MediaImageList()
	...
	#def ResetListStyle( self, with_images ):
		#self.with_images = with_images
		#list_style = wx.LC_REPORT | wx.LC_NO_HEADER | ( 0 if self.type == MMediaList.FILELIST else wx.LC_SINGLE_SEL ) | wx.LC_HRULES | wx.LC_VRULES
		#if( self.with_images ):
			#list_style = list_style | wx.LC_ICON
		#self.list.SetWindowStyleFlag( list_style )
		#self.list.Refresh()
	...
	def ItemClicked( self, event ):
		...
		if( self.filelist_selected_item_index is not None ):			
			...
			wx.CallAfter( self._click_list_item_callback, item )
	...
	def ItemSelected( self, event ):
		...
		if( self._select_list_item_callback ):
			...
			item_hash = item.Hash()
			wx.CallAfter( self._select_list_item_callback, self, item_hash )
	...
	def _BuildList( self ):
		...
		if( self._click_list_item_callback ):
			wx.CallAfter( self._click_list_item_callback, None )	#in order to reset any media buttons or anything else related to selected playlist items, see 'subtitles' media_button
		...
		for i in reversed( self.items ):
			...
			#item.SetText( i.Text() )
				...
				#item.SetBackgroundColour( "grey" )
			itemsIndex += 1
			pos = self.list.InsertItem( item )
			if( self.with_images ):
				self.list.SetItemImage( pos, self.image_list.GetImageIndex( i.media_type ) )
				logging.debug( 'MMediaList._BuildList: setting item_pos:{} to image_index:{}'.format( pos, self.image_list.GetImageIndex( i.media_type ) ) )
...
if( __name__ == '__main__' ):
	...
	class TestFrame( wx.Frame ):
		def __init__( self, parent, title ):
			...
			type = MMediaList.FILELIST
			self.l = CustomListCtrl( self._panel, style=wx.LC_REPORT|wx.LC_NO_HEADER|( 0 if type==MMediaList.FILELIST else wx.LC_SINGLE_SEL ) |wx.LC_HRULES|wx.LC_VRULES|wx.LC_ICON)
			self.l.InsertColumn( 0, 'item')
			self.l.SetColumnWidth( -1, wx.LIST_AUTOSIZE )

			self.image_list = MediaImageList()
			self.l.AssignImageList( self.image_list, wx.IMAGE_LIST_SMALL )

			items = [ ( '1', MediaType.DIR ), ( '2', MediaType.AUDIO ), ( '3', MediaType.DIR ) , ( '4', MediaType.VIDEO ), ( '5', MediaType.OTHER ), ( '6', MediaType.IMAGE ) ]
			for i in items:
				media_type = i[1]
				li = wx.ListItem()
				li.SetText( str( i[0] ) )
				pos = self.l.InsertItem( li )
				self.l.SetItemImage( pos, self.image_list.GetImageIndex( media_type ) )
				logging.debug( 'Test: setting item_pos:{} to image_index:{}'.format( pos, self.image_list.GetImageIndex( media_type ) ) )

			s = wx.BoxSizer( wx.VERTICAL )
			self._panel.SetSizer( s )
			s.Add( self.l, 1, wx.EXPAND )
