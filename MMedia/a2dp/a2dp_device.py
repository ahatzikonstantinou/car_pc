...
class A2DPDevice( Filesystem ):
	DEBUG = True
	def __init__( self, a2dp_source, abstract_device, GetSelectedListFilesCallback = None, SetPlaylistLabelTextCallback = None, GetSelectedPlaylistItemsCallback = None ):
	...
	def MainFrameInitUIFinished( self ):
		Filesystem.MainFrameInitUIFinished( self )
		self.media_player.Play( self.GetMediaStringForPlayer( None ) )
	...
	def _CreateMediaPlayer( self, video_panel_parent, control_panel_parent, video_panel_click_callback ):
		return A2DPMediaPlayer( self._playerLock, video_panel_parent, video_panel_click_callback )
	...
	def GetCurrentTrackText( self ):
		'''
		Override of Filesystem.GetCurrentTrackText because it was trying to parse a path and it was crashing
		'''
		return self.a2dp_source.name
