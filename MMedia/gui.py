...
class MMediaGui(wx.Frame):
	...
	def __init__(self, parent, title, size = (750, 420) ):	
		super(MMediaGui, self).__init__(parent, title=title, size=size)
		...
		self._media_buttons = {
			...
			'refresh': MediaButton( 'refresh', 16, None, 'refresh.png', 'refresh.png' )
		}
		...
		self.dev_builder = DeviceBuilder( 
				...
				GetSelectedPlaylistItemsCallback = self.GetSelectedPlaylistItems,
				GetSelectedSystemPlaylistItemsCallback = self.GetSelectedSystemPlaylistItems,
				playerLock = self.playerLock,
		...
		for key, device in self.devices.iteritems():
			...
			device.Deactivate()	#all devices start inactive
	...
	def OnClose( self, event ):
		#shutdown all devices. This is required by MediaRendererDevice so it can unregister the Coherence media_renderer device
		for key, device in self.devices.iteritems():
			device.Shutdown()
	...
	def ReLayoutUI( self ):
		...
		#ensure after the layout that the current device button is still visible
		if( self._current_device.Hash() in self.device_buttons.keys() ):
			device_button = self.device_buttons[ self._current_device.Hash() ]
			device_button.Show()
			device_button.SetValue( True )
			self.devPanel.ScrollChildIntoView( device_button )
	...
	def InitUI(self):
		...
		self.devPanel.SetupScrolling( scroll_x=True, scroll_y=False, rate_x=1, rate_y=1, scrollIntoView = True )
		self._AddDeviceButtons( self.devPanel, self.devSizer )
		
		#The following code is supposed to make the devPanel draggable but it does not seem to work
		#def OnMouseDrag( event ):
			#"""implement dragging"""
			#if not event.Dragging():
				#self._devPanelDragPos = None
				#return
			#self.CaptureMouse()
			#if not self._devPanelDragPos:
				#self._devPanelDragPos = event.GetPosition()
			#else:
				#pos = event.GetPosition()
				#displacement = self._devPanelDragPos - pos
				#self.devPanel.SetPosition( self.devPanel.GetPosition() - displacement )
		#self.devPanel.Bind(wx.EVT_MOTION, OnMouseDrag )

		self.devPanel_scroll = False
		def DevPanelThreadScroll( left ):
			while( self.devPanel_scroll ):
				#debug code
				#for dev_hash, b in self.device_buttons.iteritems():
					#x,y = b.GetPositionTuple()
					#r = b.GetRect()
					#logging.debug( 'Button [{}] x:[{} {}]'.format( b.GetLabel(), x, x + r.width ) )
				#end of debug code
				x,y = self.devPanel.GetViewStart()
				#pr = self.devPanel.GetRect()
				#logging.debug( 'Panel: x:[{} {}]'.format( x, x + pr.width ) )
				if( left ):
					wx.CallAfter( self.devPanel.Scroll, x-1, -1 )
				else:
					wx.CallAfter( self.devPanel.Scroll, x+1, -1 )
				time.sleep( 0.005 )				
		def OnLeftScroll( event ):
			self.devPanel_scroll = True
			th = Thread( target = DevPanelThreadScroll, args=[True], group = None )
			th.start()
		def OnRightScroll( event ):
			self.devPanel_scroll = True
			th = Thread( target = DevPanelThreadScroll, args=[False], group = None )
			th.start()
		def OnStopScroll( event ):
			self.devPanel_scroll = False
		self.leftDevScrollButton = wx.BitmapButton( dev_container_panel, bitmap = GetImage( 'navigate_left.png', '16x16' ) )
		self.leftDevScrollButton.Bind( wx.EVT_LEFT_DOWN, OnLeftScroll )
		self.leftDevScrollButton.Bind( wx.EVT_LEFT_UP, OnStopScroll )
		self.rightDevScrollButton = wx.BitmapButton( dev_container_panel, bitmap = GetImage( 'navigate_right.png', '16x16' ) )
		self.rightDevScrollButton.Bind( wx.EVT_LEFT_DOWN, OnRightScroll )
		self.rightDevScrollButton.Bind( wx.EVT_LEFT_UP, OnStopScroll )
		...
		rightDisplaySizer.Add( self.radioPanel, 6, wx.EXPAND | wx.ALIGN_CENTER, border = 0 )
	...
	def _StartDevPanelDrag( self, event ):
		self._start_devPanel_drag = True
		
	def _StopDevPanelDrag( self, event ):
		self._start_devPanel_drag = False
		
	def _DevPanelDrag( self, event ):
		if( self._start_devPanel_drag ):
			logging.debug( 'Dragging devPanel...' )
	...
	def _OnDevicePreampChanged( self, preamp ):
		'''
		Called by the settings_control.audio_panel when the user changes the preamp level
		'''
		self._current_device.state.SetPreamp( preamp )
		#update current device's volume, so that the new preamp is taken into account
		self._current_device.SetVolume( self.volumeControl.volume )

	def _OnDeviceBrightnessChanged( self, brightness ):
		self._current_device.SetBrightness( brightness )
	
	def _OnDeviceContrastChanged( self, contrast ):
		self._current_device.SetContrast( contrast )
	
	def _OnDeviceGammaChanged( self, gamma ):
		self._current_device.SetGamma( gamma )
	
	def _OnDeviceHueChanged( self, hue ):
		self._current_device.SetHue( hue )
	
	def _OnDeviceSaturationChanged( self, saturation ):
		self._current_device.SetSaturation( saturation )
		...
		if( update_dev_buttons ):
			for c in self.devPanel.GetChildren():
				if( c.device.Hash() == device_hash ):
					...
					#c.SetValue( True )
					c.Press()
					#logging.debug( 'MMedia._SetCurrentDeviceByHash will scroll into view {}'.format( c.GetLabel() ) )
					#self.devPanel.ScrollChildIntoView( c )
					#logging.debug( 'MMedia._SetCurrentDeviceByHash scrolled into view {}'.format( c.GetLabel() ) )
					break
	
	def _SetCurrentDeviceByDevice( self, device ):
		settings_panel_is_hidden = self.settings_control.IsShown()
		...
		self._RefreshLists( self.lists_panel )
		
		self._ConnectDeviceWithSettingsControl( self._current_device )

		logging.debug( 'GUI._SetCurrentDeviceByDevice will set volume to {} for device {}'.format( self.State.GetVolume(), self._current_device.name ) )
		...
		#make sure that if the settings panel was visible with the previous device, it is still visible
		self.OnSettings( show_settings = settings_panel_is_hidden )
	...
	def _ConnectDeviceWithSettingsControl( self, device ):
		#image
		if( device.HasVideoSettings() ):
			self.settings_control.ShowImageSettings()
			
			self.settings_control.img_panel.SetBrightnessCallback = None
			self.settings_control.img_panel.SetBrightness( self._current_device.state.GetBrightness() )
			self.settings_control.img_panel.SetBrightnessCallback = self._OnDeviceBrightnessChanged

			self.settings_control.img_panel.SetContrastCallback = None
			self.settings_control.img_panel.SetContrast( self._current_device.state.GetContrast() )
			self.settings_control.img_panel.SetContrastCallback = self._OnDeviceContrastChanged

			self.settings_control.img_panel.SetGammaCallback = None
			self.settings_control.img_panel.SetGamma( self._current_device.state.GetGamma() )
			self.settings_control.img_panel.SetGammaCallback = self._OnDeviceGammaChanged

			#self.settings_control.img_panel.SetHueCallback = None
			#self.settings_control.img_panel.SetHue( self._current_device.state.GetHue() )
			#self.settings_control.img_panel.SetHueCallback = self._OnDeviceHueChanged
#
			#self.settings_control.img_panel.SetSaturationCallback = None
			#self.settings_control.img_panel.SetSaturation( self._current_device.state.GetSaturation() )
			#self.settings_control.img_panel.SetSaturationCallback = self._OnDeviceSaturationChanged
		else:
			self.settings_control.HideImageSettings()
		
		#audio
		self.settings_control.audio_panel.SetPreampCallback = None
		self.settings_control.audio_panel.SetPreamp( self._current_device.state.GetPreamp() )
		self.settings_control.audio_panel.SetPreampCallback = self._OnDevicePreampChanged
	...
	def OnSettings( self, event = None, show_settings = False ):
	...
	def _EnsureDeviceButtonIsVisible( self, button ):
		bx,by = button.GetPositionTuple()
		r = button.GetRect()
		logging.debug( 'MMedia._EnsureDeviceButtonIsVisible: Button [{}] [{} {}]'.format( button.GetLabel(), bx, bx + r.width ) )
		
		bx1 = bx
		bx2 = bx + r.width
		
		px1,py = self.devPanel.GetViewStart()
		pr = self.devPanel.GetRect()
		px2 = px1 + pr.width
		logging.debug( 'MMedia._EnsureDeviceButtonIsVisible: Panel: x:[{} {}]'.format( px1, px2 ) )
		
		#if bx1 >= 0 and bx2 <= px2 nothing to do
		#if bx1 < 0  scroll right
		#if bx2 > px2 scroll left
		
		left = True
		if( bx1 < 0 ):
			left = True
			logging.debug( 'MMedia._EnsureDeviceButtonIsVisible: button_x1:{} < 0 Will scroll left'.format( bx1 ) )
		elif( bx2 > px2 ):
			left = False
			logging.debug( 'MMedia._EnsureDeviceButtonIsVisible: button_x2:{} > panel_x2:{} Will scroll right'.format( bx2, px2 ) )
		else:
			logging.debug( 'MMedia._EnsureDeviceButtonIsVisible: button [{}] is visible, nothing to do'.format( button.GetLabel() ) )
			return
			
		if( left ):
			wx.CallAfter( self.devPanel.Scroll, px1+bx1, -1 )
		else:
			wx.CallAfter( self.devPanel.Scroll, px1+(bx2-px2), -1 )
		
	def _BuildLists( self, buttons_panel, lists_panel, with_images = False ):
		...
		self.filelist_widget = self._BuildFilelist( lists_panel, with_images )
		...
		self.playlist_widget = self._BuildPlaylist( lists_panel, with_images )
		...
		self.system_playlists_widget = self._BuildSystemPlaylists( lists_panel, with_images )
	...
	def _SetDeviceActiveList( self, device, active_list ):
		list_id = -1
		if( active_list == self.filelist ):
			list_id = 0
		elif( active_list == self.playlist ):
			list_id = 1
		elif( active_list == self.system_playlists ):
			list_id = 2
		else:
			return	
		device.state.SetActiveListId( list_id )
		
	def GetDeviceActiveList( self, device ):
		list_id = device.state.GetActiveListId()
		if( list_id == 0 ):
			return self.filelist
		elif( list_id == 1 ):
			return self.playlist
		elif( list_id == 2 ):
			return self.system_playlists

		return None
		
	def GetDeviceActiveListWidget( self, device ):
		list_id = device.state.GetActiveListId()
		if( list_id == 0 ):
			return self.filelist_widget
		elif( list_id == 1 ):
			return self.playlist_widget
		elif( list_id == 2 ):
			return self.system_playlists_widget

		return None
			
	def _RefreshLists( self, lists_panel ):
		logging.debug( 'gui._RefreshLists starting for device {}...'.format( self._current_device.name ) )

		#we need to recreate the lists to accomodate the display of images next to each list item
		with_images = self._current_device.UseMediaListsWithImages()

		#self.filelist.ResetListStyle( with_images )
		#self.playlist.ResetListStyle( with_images )
		#self.system_playlists.ResetListStyle( with_images )
		self.filelist_widget.Destroy()
		self.playlist_widget.Destroy()
		self.system_playlists_widget.Destroy()
		self.filelist_widget = self._BuildFilelist( lists_panel, with_images )
		self.filelist_button.associated_object = self.filelist
		self.filelist_button.associated_widget = self.filelist_widget
		self.filelist_button.panel = self.filelist_widget
		self.playlist_widget = self._BuildPlaylist( lists_panel, with_images )
		self.playlist_button.associated_object = self.playlist
		self.playlist_button.associated_widget = self.playlist_widget
		self.playlist_button.panel = self.playlist_widget
		self.system_playlists_widget = self._BuildSystemPlaylists( lists_panel, with_images )
		self.system_playlists_button.associated_object = self.system_playlists
		self.system_playlists_button.associated_widget = self.system_playlists_widget
		self.system_playlists_button.panel = self.system_playlists_widget
		...
		#Determine which device_media_list to show
		...
		#If I show no widgets, no list is supported by this device
		if( shown_widgets_count == 0 ):
			self.lists_panel.Hide()
			self.list_buttons_panel.Hide()
		else:
			self.lists_panel.Show()
			#If am showing just one list, this is my active one
			if( shown_widgets_count == 1 ):
				self.list_buttons_panel.Hide()
				self._SetDeviceActiveList( self._current_device, self.current_active_list )
			else:
				self.list_buttons_panel.Show()
				#If I am showing many lists, 
				#the active one is either the one save in the current devices state as active
				if( self._current_device.state.GetActiveListId() != -1 ):
					device_media_list = self.GetDeviceActiveList( self._current_device )
					if( device_media_list == self.playlist ):
						self.playlist_button.SetValue( True )
					elif( device_media_list == self.filelist ):
						self.filelist_button.SetValue( True )
				else:	#or the playlist by default if no active list is saved in the device's state e.g. application start
	...
	def _BuildFilelist( self, parent_panel, with_images ):
		self.filelist = MMediaList( parent_panel, self.Settings, type = MMediaList.FILELIST, 
			select_list_item_callback = self.PlayTrackFromList, actions = MMediaGui._filelist_functions, with_images = with_images )
		#print( 'filelist was built' )
		return self.filelist.GetWidget()
		
	def _BuildPlaylist( self, parent_panel, with_images ):
		self.playlist = MMediaList( parent_panel, self.Settings, 
			...
			click_list_item_callback = self.OnClickedNowPlayingItem, actions = MMediaGui._playlist_functions, with_images = with_images )
		#print( 'playlist was built' )
		return self.playlist.GetWidget()
		
	def _BuildSystemPlaylists( self, parent_panel, with_images ):
		self.system_playlists = MMediaList( parent_panel, self.Settings, 
			type = MMediaList.SYSTEM_PLAYLISTS, 
			with_list_title = False, 
			select_list_item_callback = self.PlayTrackFromList, 
			...
			for i in range( 0, deviceSettings.GetSpeedDialRows() ):
				...
				for k in range( 0, deviceSettings.GetSpeedDialButtonsPerRow() ):
					...
					if( speed_dial_button_count >= speed_dial_button_total_count ):
						...
						#speeddial context menu pops up with right click now
						#stationButton.Bind( wx.EVT_LEFT_DOWN, self.SetSpeedDialStart )
						#stationButton.Bind( wx.EVT_LEFT_UP, self.SetSpeedDialStop )
						stationButton.Bind( wx.EVT_RIGHT_UP, self.speeddial_popup.OnShowPopup )
	...
	def SpeedDialBack( self, event=None ):
		wx.CallAfter( self.ZappStart, callback=self.SpeedDialBack )
		wx.CallAfter( self.PushNextSpeedDialButton, False )
		
	def SpeedDialForward( self, event=None):
		wx.CallAfter( self.ZappStart, callback=self.SpeedDialForward )
		wx.CallAfter( self.PushNextSpeedDialButton, True )
		
	def SetCurrentActiveList( self, device_media_list ):
		self.current_active_list = device_media_list
		self._SetDeviceActiveList( self._current_device, device_media_list )
		
	def PlayTrackFromList( self, device_media_list, track_hash ):
		self.current_active_list = device_media_list
		self._SetDeviceActiveList( self._current_device, device_media_list )
		self.PlayTrack( track_hash )

	def PlayTrackPreviousListStation( self, event=None):
		wx.CallAfter( self.ZappStart, callback=self.PlayTrackPreviousListStation )
		wx.CallAfter( self._ListSelectNext, False )
		
	def PlayTrackNextListStation( self, event=None ):
		wx.CallAfter( self.ZappStart, callback=self.PlayTrackNextListStation )
		wx.CallAfter( self._ListSelectNext, True )
	...
	def _ListSelectNext( self, forward ):
		...
		if( self.current_active_list ):
			wx.CallAfter( self.current_active_list.ListSelectNext, forward )
	...
	def GetSelectedPlaylistItems( self ):
		items = []
		if( self.playlist ):
			#print( 'got playlist' )
	...
	def SetSpeedDialStop( self, event ):
		print( 'SetSpeedDialStop! IsRunning set:{}'.format( 
	...
	def ClearSpeedDialButton( self, button ):
		...
		logging.debug( 'Cleared speed dial {}'.format( index ) )
	...
	def ToggleZapp( self, event ):
		...
		button = event.GetEventObject()
		img = GetImage( 'no_flash.png' )
		if( self.zapp_is_on ):
			img = GetImage( 'flash.png' )
		if( isinstance( button, wx.StaticBitmap ) ):
			wx.CallAfter( button.SetBitmap, img )
		else:
			wx.CallAfter( button.SetBitmapLabel, img )
	...
	#Gu functions to be called by devices or other code to update the GUI, identified by GU_prefix i.e. GUI Update
	def GU_IsPlaying( self ):
		...
		#check to see if should enable the "select subtitles" button
	...
	def GU_DeviceShowMe( self, device_hash ):
		'''
		This function so far is only called by the UPNP Media renderer which needs to always be active but it will be displayed only when playing.
		'''
		self._SetCurrentDeviceByHash( device_hash, True )
		
	def GU_DeviceHideMe( self, device_hash ):
		'''
		This function so far is only called by the UPNP Media renderer which needs to hide when it stops playing.
		'''
		if( not self.devices.has_key( device_hash ) ):
			logging.debug( 'MMediaGUI was asked to hide device with hash {} but it''s not in my devices'.format( device_hash ) )
			return
		
		if( device_hash ):
			button = self.device_buttons[ device_hash ]
			button.Hide()
			activate_another_device = False
			if( self._current_device and self._current_device == self.devices[ device_hash ] ):
				activate_another_device = True
			if( activate_another_device ):
				if( len( self.devices.keys() ) > 0 ):
					key = self.devices.keys()[0]
	...
	def GU_SetMediaButtonPressed( self, button_name, set_pressed ):
		media_button = self.GetMediaButtonByFunctionName( button_name )
		if( media_button ):
			media_button.is_pressed = set_pressed
			self.RefreshMediaButton( media_button )
		
	def GU_GetMediaButtonIsPressed( self, button_name ):
		media_button = self.GetMediaButtonByFunctionName( button_name )
		if( media_button ):
			return media_button.is_pressed
		return False
	
	def GU_ShowList( self, list_widget ):
		'''
		So far this function is called by MediaDevice.Playtrack to display the playing now list if supported.
		'''
		if( list_widget == self.filelist_widget ):
			self.filelist_button.Press()
		elif( list_widget == self.playlist_widget ):
			self.playlist_button.Press()
		elif( list_widget == self.system_playlists_widget ):
			self.system_playlists_button.Press()
		elif( list_widget == self.numpad_panel ):
			self.numpad_button.Press()
		else:
			raise Exception( 'Illegal list widget: {}. Valid list widgets are filelist_widget, playlist_widgetsystem_playlists_widget, self.numpad_panel'.format( button_name ) )
	#End of Gu functions
...
class MarqueeText:
	...
	def StartMarquee( self ):
		self.StopMarquee()
		logging.debug( 'marquee_timer started with marquee text:{}'.format( self.text.encode( 'utf-8' ) ) )
		self.marquee_timer.Start( 150, False )
		
	def StopMarquee( self ):
		if( self.marquee_timer.IsRunning() ):
			logging.debug( 'marquee_timer stopped with marquee text:{}'.format( self.text.encode( 'utf-8' ) ) )
...
def GetImage( imageFile, size = '48x48' ):
	...
	if( imageFile.startswith( 'no_' ) ):
		image = wx.Image( imageDir + imageFile[3:], wx.BITMAP_TYPE_ANY )
	else:
		image = wx.Image( imageDir + imageFile, wx.BITMAP_TYPE_ANY )
...
class StereoMonoPanelObserver:
	...
	def IsStereo( self, isStereo ):
		wx.CallAfter( self.volume_control.SetStereo, isStereo )
...
class NumpadTrackHashObserver:
	...
	def CurrentTrackHashIs( self, track_hash ):
		print( 'NumpadTrackHashObserver updated with track_hash: {}'.format( track_hash.encode( 'utf-8' ) ) )
		wx.CallAfter( self.numText.SetLabel, track_hash )
...
class PlaylistTrackHashObserver:
	...
	def CurrentTrackHashIs( self, track_hash ):
		...
		print( '\twill try to select track {} in list'.format( track_hash.encode( 'utf-8' ) ) )
		if( self.playlist ):
			print( '\t\twill definitely select track {} in list'.format( track_hash.encode( 'utf-8' ) ) )
...
if __name__ == '__main__':
	...
	app = wx.App()
	MMediaGui(None, title="MMedia", size = (750, 420) )
	app.MainLoop()
