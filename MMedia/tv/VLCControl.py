...
class VLCControl():
	...
	def __init__(self, videoParentPanel, controlsParentPanel, state, playerLock, stateSaveCallback = None, 
		videoPanelClickCallback = None, hideDevice = False ):
		...
		#self.videopanel.SetBackgroundColour(wx.GREEN)	#for testing
		...
		print( 'Will try to bind videopanel with {}'.format( self.videoPanelClickCallback ) )			
		if( self.videoPanelClickCallback is not None ):
			print( 'binding videopanel with {}'.format( self.videoPanelClickCallback ) )
			self.videopanel.Bind( wx.EVT_LEFT_UP, self.videoPanelClickCallback )
