...
#If while the app is running, a MediaServer goes down and up again, the response I get for browsing it's root dir is 501 error. Probably the server is not ready yet. If I do it with the debugger (slight delay for pressing F6 for next) the response is fine. Therefore I wait and retry 
RETRIES_BROWSE_501ERROR = 3
SECS_FOR_RETRY_BROWSE_501ERROR = 0.5
...
class UPnPProvider:
	...
	'''
	DEBUG = False
	REMOVE_HOST_NORESPONSSECS = 300
	NORESPONSE_TICKSECS = 80
	SEARCH_SECS = 20
...
class MSearchListenThread( threading.Thread ):
	...
	def run( self ):
		while True:
			try:
				data = self._upnp.recv( 1024, self._server )
				if( self._upnp.parseSSDPInfo( data, False, False ) ):
					if( self.DEBUG ):
						logging.debug( 'UPnPProvider.MSearchListenThread: Got a host' )
				#else:
					#logging.debug( 'UPnPProvider.MSearchListenThread: Got something unacceptable:\n{}'.format( data ) )
				time.sleep( 0.1 )								
			except Exception, e:
				pass
...
def MediaServerContentDirectory( upnp_proxy, hostInfo, device_name, item_id, browse_flag = 'BrowseDirectChildren', requested_count = 0 ):
	...
	try:
		...
		while True:
			...
			if( response == False or ( '<s:Fault>' in response and '<errorCode>501</errorCode>' in response ) ):
				got_error = True
				
			if( got_error ):
				logging.debug( 'upnp_provider.MediaServerContentDirectory got error ({}).'.format( retries ) )
				if( retries < RETRIES_BROWSE_501ERROR ):
					logging.debug( 'upnp_provider.MediaServerContentDirectory will retry after {} seconds.'.format( SECS_FOR_RETRY_BROWSE_501ERROR ) )
					retries += 1
					time.sleep( SECS_FOR_RETRY_BROWSE_501ERROR )
				else:
					logging.debug( 'upnp_provider.MediaServerContentDirectory reached max retries and will abandon the attempt' )
					return None
			else:
				'''
				NOTE: This response.decode( 'utf-8' ).encode( 'utf-8' ) is VERY WEIRD. If I don't do encode( 'utf-8' ) greek characters crash. If I don't do encode( 'utf-8' ) some latin characters crash. So I need to do them both to avoid crashes. Also the next two commented lines work too.
				'''
				result = got_result( first_level_dict( etree.fromstring( response.decode( 'utf-8' ).encode( 'utf-8' ) ).find( '*//{urn:schemas-upnp-org:service:ContentDirectory:1}BrowseResponse' ) ) )
				#parser = etree.XMLParser(ns_clean=True, recover=True, encoding='utf-8')
				#result = got_result( first_level_dict( etree.fromstring( response.decode( 'utf-8' ).encode( 'utf-8' ), parser=parser ).find( '*//{urn:schemas-upnp-org:service:ContentDirectory:1}BrowseResponse' ) ) )
				#for l in result:
