...
class upnp:
	...
	def __init__( self, ip, port, iface, appCommands = None, HostAddedCallback = None, HostRemovedCallback = None, DeviceAddedCallback = None, DeviceRemovedCallback = None, acceptable_devices = None, max_noresponse_secs = MAX_NORESPONSE_SECS ):
		if appCommands:
			self.completer = CmdCompleter(appCommands)
		self._lock = threading.RLock()
		self._acceptable_devices = acceptable_devices
		self._knownHeaders = upnp.KNOWNHEADERS
		self.HostAddedCallback = HostAddedCallback
		self.HostRemovedCallback = HostRemovedCallback
		self.DeviceAddedCallback = DeviceAddedCallback
		self.DeviceRemovedCallback = DeviceRemovedCallback
		
		self._max_noresponse_secs = max_noresponse_secs
	...
	def RemoveHost( self, host ):
		with self._lock:
			try:
				...
				for hostInfo in self.ENUM_HOSTS:
					if( hostInfo['name'] == host ):
						if( self.HostRemovedCallback ):
							self.HostRemovedCallback( hostInfo['name'] )
						if( self.DeviceRemovedCallback ):
							for deviceName, deviceStruct in hostInfo['deviceList'].items():
								self.DeviceRemovedCallback( host, deviceName )
						del self.ENUM_HOSTS[ hostID ]
						logging.debug( 'removed host: {}'.format( str( hostID ) ) )
						self.ListHosts()
						return True
					hostID += 1
				logging.debug( 'host not found, maybe already removed' )
				return False
			except:
				logging.error( 'failed to remove host {}'.format( hostID ) )
				return False
	
	#Lock
	def RemoveDevice( self, host, device ):
		with self._lock:
			try:
				hostID = 0
				#for hostID, hostInfo in self.ENUM_HOSTS.items():
				for hostInfo in self.ENUM_HOSTS:
					if( hostInfo['name'] == host ):
						if( device in hostInfo['deviceList'].keys() ):
							del hostInfo['deviceList'][device]
							logging.debug( 'removed device: {}'.format( device ) )
							if( self.DeviceRemovedCallback ):
								self.DeviceRemovedCallback( host, device )
							return True
						logging.debug( 'device not found, maybe already removed' )
						return False
					hostID += 1
				logging.debug( 'host not found, maybe already removed' )
				return False
			except:
				logging.error( 'failed to remove device {} of host {}'.format( device, hostID ) )
				return False
				
	#Lock
	def RemoveService( self, host, service ):
		with self._lock:
			try:
				hostID = 0
				#for hostID, hostInfo in self.ENUM_HOSTS.items():
				for hostInfo in self.ENUM_HOSTS:
					if( hostInfo['name'] == host ):
						for deviceName, deviceStruct in hostInfo['deviceList'].items():
							if( service in self.ENUM_HOSTS[ hostID ]['deviceList'][ deviceName ]['services'].keys() ):
								del self.ENUM_HOSTS[ hostID ]['deviceList'][ deviceName ]['services'][service]
								logging.debug( 'removed service: {}'.format( service ) )
								return True
						logging.debug( 'service not found, maybe already removed' )
						return False
					hostID += 1
				logging.debug( 'host not found, maybe already removed' )
				return False
			except:
				logging.error( 'failed to remove service {} of host {}'.format( service, hostID ) )
				return False
	...
	def ListHosts( self ):
		with self._lock:
			hostID = 0
			for hostInfo in self.ENUM_HOSTS:
				logging.info( '{}). {}'.format( hostID, hostInfo['name'] ) )
				hostID += 1
	...
	def parseSSDPInfo(self,data,showUniq,verbose):
		with self._lock:
			...
			page = False
			upnpType = None
			knownHeaders = self._knownHeaders
			...
			if messageType != False:
				...
				nt = self.parseHeader( data, "NT" )
				nts = self.parseHeader( data, "NTS" )
				(host,page) = self.parseURL(xmlFile)
				
				#logging.debug( 'received packet:\n{}'.format( data ) )
				#logging.debug( 'received\n\thost:{}\n\tupnpType:{}\n\tnts:{}\n\tnt:{}\n\tpage:{}'.format( host, upnpType, nts, nt, page ) )
				#if( not Nts ):
					#logging.debug( 'nts:false, received packet:\n{}'.format( data ) )
				
				#Sanity check to make sure we got all the info we need
				#logging.debug( 'xmlFile == False: {}, host == False: {}, page == False: {}'.format( (xmlFile == False), ( host == False ), (page == False) ) )
				...
				if( nts == 'ssdp:byebye' ):
					if( nt == 'upnp:rootdevice' ):
						logging.debug( 'Will remove host {}'.format( host ) )
						return self.RemoveHost( host )
					else:
						device = self.parseDeviceTypeName( nt )
						if( device ):
							return self.RemoveDevice( host, device )
						else:
							service = self.parseServiceTypeName( nt )
							logging.debug( 'Will remove service {} of host {}, nt: {}'.format( service, host, nt ) )
							if( service ):
								return self.RemoveService( host, service )
							else:
								return False
					return False
				#else:
					#logging.debug( 'refreshed host {}'.format( host ) )
					

				#Get the protocol in use (i.e., http, https, etc)
				protocol = xmlFile.split('://')[0]+'://'

				#logging.debug( 'will process host {}'.format( host ) )
				...
				for hostInfo in self.ENUM_HOSTS:
					#logging.debug( 'looking for host {}, comparing with {} ...'.format( host, hostInfo['name'] ) )
					if( hostInfo['name'] == host ):
						hostFound = True
						hostInfo['last_seen'] = time.time()
						if self.UNIQ:
							self.getHostInfoByName( host )
							if( self.HostAddedCallback ):
								self.HostAddedCallback( host )
							return False

				logging.debug( 'self.UNIQ: {}, hostFound: {}'.format( self.UNIQ, hostFound ) )
				if (hostFound and not self.UNIQ) or not hostFound:
					if( self.DEBUG ):
						logging.debug( 'will add host: {}'.format( host ) )
					#if( self._acceptable_devices ):
						#hostInfo = self.getHostInfoByName( host )
						#if not ( set( hostInfo['deviceList'] ) & set( self._acceptable_devices ) ):
							#logging.debug( 'this is an unacceptable device' )
							#return False
					#Get the new host's index number and create an entry in ENUM_HOSTS
					index = len(self.ENUM_HOSTS)
					#self.ENUM_HOSTS[index] = {
					self.ENUM_HOSTS.append(
								{
									'name' : host,
									'dataComplete' : False,
									'proto' : protocol,
					...
					if( self.HostAddedCallback ):
						self.HostAddedCallback( host )
					if( self.DEBUG ):
						logging.debug( 'added new host {} at index {}'.format( host, index ) )
					self.getHostInfoByName( host )
					if( self.DEBUG ):
						self.ListHosts()
	...
	def sendSOAP(self,hostName,serviceType,controlURL,actionName,actionArguments):
		...
		for arg, ( val, dt, pos ) in sorted( actionArguments.items(), key=lambda a:a[1][2] ):
			#print ( '{}: pos:{}, val:{}, dt:{}'.format( arg, pos, val, dt ) )
		...
		soapBody = 	'<?xml version="1.0"?>\n'\
				...
				'<s:Body><ns0:%s>%s</ns0:%s></s:Body></s:Envelope>' % (serviceType,actionName,argList,actionName)
		...
		try:
			...
			if self.DEBUG:
				...
				#print( 'received data: {}'.format( h.unescape( data ) ) )
				if not data:
					break
				else:
					soapResponse += data
			...
			if not header.upper().startswith('HTTP/1.') and ' 200 ' in header.split('\r\n')[0]:
				...
				#logging.debug( 'sendSoap returns body of response:\n{}'.format( body ) )
				return body
                except Exception, e:
				...
				for deviceName,deviceStruct in hostInfo['deviceList'].iteritems():
					...
					for serviceName,serviceStruct in deviceStruct['services'].iteritems():
						...
						for actionName,actionStruct in serviceStruct['actions'].iteritems():
							...
							for argName,argAttributes in actionStruct['arguments'].iteritems():
								fp.write('\t\t\t\t\t%s at position %d\n' % ( argName, argAttributes.position ) )
								#import pprint; pprint.pprint( argAttributes )
								argStruct = argAttributes.tags
	...
	#Lock
	def getHostInfoByName( self, hostname ):
		with self._lock:
			hostID = 0
			#for hostID,hostInfo in self.ENUM_HOSTS.iteritems():
			for hostInfo in self.ENUM_HOSTS:
				if hostInfo['name'] == hostname:
					...
					try:
						...
						if hostInfo != False:
							if( self.DEBUG ):
								logging.debug( "Requesting device and service info for {} (this could take a few seconds)...".format( hostInfo['name'] ) )
							if hostInfo['dataComplete'] == False:
								(xmlHeaders,xmlData) = self.getXML(hostInfo['xmlFile'])
								if xmlData == False:
									logging.error( 'Failed to request host XML file: {}'.format( hostInfo['xmlFile'] ) )
									return None
								if self.getHostInfo(xmlData,xmlHeaders,hostID) == False:
									logging.error( "Failed to get device/service info for %s..." % hostInfo['name'] )
									return None
							if( self.DEBUG ):
								logging.debug( 'Host data enumeration complete!' )
							return self.ENUM_HOSTS[hostID]
	...
	def parseDeviceInfo(self,xmlRoot,index):
		with self._lock:
			...
			for device in xmlRoot.getElementsByTagName(devTag):
				...
				if( self.DeviceAddedCallback ):
					self.DeviceAddedCallback( self.ENUM_HOSTS[index], deviceDisplayName )
			...
			try:
				...
				for action in actions:
					...
					for argument in arguments:
						...
						class SortedArgument:
							def __init__( self, name, pos ):
								self.name = name
								self.position = pos
								self.tags = {}
								
						service['actions'][actionName]['arguments'][argName] = SortedArgument( argName, pos )
						#service['actions'][actionName]['arguments'][argName] = {}
						#service['actions'][actionName]['arguments'][argName]['position'] = pos
						#service['actions'][actionName]['arguments'][argName]['tags'] = {}

						#print( 'doing argument {} at pos {}'.format( argName, pos ) )
						pos += 1
						...
						for tag in argTags:
							try:
								...
								#service['actions'][actionName]['arguments'][argName]['tags'][tag] = str(argument.getElementsByTagName(tag)[0].childNodes[0].data )
								service['actions'][actionName]['arguments'][argName].tags[tag] = str(argument.getElementsByTagName(tag)[0].childNodes[0].data )
							except:
	...
	#Called in order to tell upnp that another tick (i.e. time period)
	#has occured, and that may mean that hosts who have not called in
	#may be considered down and removed
	#Lock
	def NoResponseTick( self ):
		with self._lock:
			#for hostID,hostInfo in self.ENUM_HOSTS.items():
			for hostInfo in self.ENUM_HOSTS:
				now = time.time()
				if( now - hostInfo['last_seen'] > self._max_noresponse_secs ):
					logging.debug( 'Host {} reached _max_noresponse_secs:{}, will be removed.'.format( hostInfo['name'], self._max_noresponse_secs ) )
					self.RemoveHost( hostInfo['name'] )
		...
		if action == 'list':
			...
			if argc != 6:
				...
				try:
					...
					sorted_args = [ x for x in sorted( actionArgs.values(), key=operator.attrgetter( 'position' ) ) ]
					#import pprint; pprint.pprint( sorted_arg_names )
					#print( 'sorted_arg_names: {}'.format( sorted_arg_names ) )
				...
				#for argName,argAttributes in actionArgs.iteritems():
				for arg in sorted_args:
					argName = arg.name
					argVals = arg.tags
					#argVals = argAttributes['tags']
					...
					if argVals['direction'].lower() == 'in':
						print( 'Argument position {}'.format( arg.position ) )
						...
						try:
							...
							sendArgs[argName] = (uInput.strip(),stateVar['dataType'], arg.position)
						except KeyboardInterrupt:
							print ""
							return
						print ''
