...
class UPnPMediaServerDevice( Filesystem ):
	def __init__( self, upnp_hostInfo, upnp_device_name, upnp_provider, abstract_device, GetSelectedListFilesCallback = None, SetPlaylistLabelTextCallback = None, GetSelectedPlaylistItemsCallback = None  ):
		...
		Filesystem.__init__( self, 
			...
			GetSelectedPlaylistItemsCallback = GetSelectedPlaylistItemsCallback 
		)
	...
	def DoPlayTrack( self, track_hash ):
		...
		Filesystem.DoPlayTrack( self, track_hash )
	...
	def _ReadDir( self, directory ):
		...
		if( directory != self.root_dir ):
			try:
				#Check my known items in order to avoid errors regarding leading slashes in the object id
				item = self.GetMediaItem( directory )
				if( item ):
		...
		if( items ):
			...
			#Just show an error
			self._media_items = [ MMediaListItem( hash = '', text = 'No items found', is_enabled = False ) ]	
	...
	def GetSubtitlesDialog( self, dialog_parent, SelectedSubtitlesCallback ):
		...
		if( self.current_media_item is not None ):
			subs_list = self.current_media_item.GetSubtitles()
			
		return UPnPSubtitlesDlg( dialog_parent, SelectedSubtitlesCallback, subs_list, self.GetCurrentSubtitle() )
		
	def SubtitlesAreAvailable( self ):
		logging.debug( 'This is UPnPMediaServerDevice.SubtitlesAreAvailable' )
		if( self.current_media_item is None ):
			return False
			
		return self.current_media_item.HasSubtitles()
