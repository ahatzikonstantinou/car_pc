...
class UPnPMediaRendererPlayer( MediaPlayer ):
	...
	def __init__( self, video_parent_panel, playerLock, videoPanelClickCallback = None ):
		...
		self.Instance = self.CreateInstance( True )
	...
	def DoPlay( self ):
		'''
		We need to override this function in order to avoid the destruction and recreation of vlc.Instance and
		vlc.Player
		'''
		with self.playerLock :
			...
			if( self.player.play() == -1 ):
				logging.debug( 'Cannot play "{0}"'.format( self.current_media_str ) )
			else:
				logging.debug( 'playing {}:{}'.format( self.current_media_str, self.current_media_options ) )
				self.PrintInfo()
