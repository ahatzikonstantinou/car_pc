...
AT={
    ...
    'i0': 'ATI0',
    'i1': 'ATI1',
    'mid': 'AT+CGMI', #Request Manufacturer Identification
    'lid' : 'AT+CGMM', #Request Model Identification
    'version': 'AT+CGMR',
    'imei1': 'AT+CGSN',
    'imei2': 'AT+GSN',
    'model': 'AT+GMM',
    'calls': 'AT+CLCC', #List current calls
    'snum': 'AT+CNUM', # Subscriber number
    'charset': 'AT+CSCS?', #returns the current value of the active character set.
    'set_charset': 'AT+CSCS=%s\r\n', #returns the current value of the active character set.
    'oper': 'AT+COPS?' #Operator Selection
   }
okay='OK\r\n'
...
class phone:
    ...
    def runCommandDebug( self, at_command, *at_args ):
        self.port.write(AT[at_command]%at_args)
        sleep(1)
        response=self.port.readlines()
        return response
    ...
    def loadPhonebook(self,simcard=False):
        ...
        lines = self.runCommand('pb_number') 
        print( lines )  #includes \r\n lines
        line = [ x for x in lines if RE['pb_number'].match( x ) ] #this is our line
        if( len( line ) != 1 ):
          print( 'Error!' )
          raise ATError( '{} lines found corresponding to phonebook answers'.format( len( line ) ) )
        print( line )
        report = RE['pb_number'].match( line[0] ).groups()
        print( report )
        #return
        ...
        self.runCommand( 'set_charset', '"UTF-8"' )
        ...
        for line in lines:
            ...
            if parsed_line:
                ...
                print( 'name:{},number:{},type:{}'.format( name,number,type ) )
        self.phonebook=pb        
...
if __name__=="__main__":
    ...
    #for cmd in AT:
      #print( cmd )
      #
    #print( p.runCommandDebug( 'pb_number' ) )
    #print( p.runCommandDebug( 'version' ) )
    #print( p.runCommandDebug( 'imei1' ) )
    #print( p.runCommandDebug( 'imei2' ) )
    #print( p.runCommandDebug( 'pb_number' ) )
    #print( p.runCommandDebug( 'model' ) )
    p.loadPhonebook()
