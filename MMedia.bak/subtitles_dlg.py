...
class UPnPSubtitlesDlg( SubtitlesDlg ):
	def __init__( self, parent, SelectedSubtitlesCallback, upnp_media_item ):
		SubtitlesDlg.__init__( self, parent, SelectedSubtitlesCallback )
		self.upnp_media_item = upnp_media_item
		self.subtitle_items = []
		
	def _GetSubtitles( self ):
		self.subtitle_items = self.upnp_media_item
	...
	def ShowDialog( self ):
		class Dlg( wx.Dialog ):
			def __init__( self, parent, title ):
				...
				self.subs_list = CustomListCtrl( self._panel, style=wx.LC_REPORT|wx.LC_NO_HEADER| wx.LC_SINGLE_SEL |wx.LC_HRULES|wx.LC_VRULES)
				self.subs_list.InsertColumn( 0, 'Subtitles')
				self.subs_list.SetColumnWidth( -1, wx.LIST_AUTOSIZE )
				for i in range( self.media_player.video_get_spu_count(), 1 ):
					self.subs_list.InsertItem( i )
				sizer.Add( self.subs_list )
				...
				sizer.Add(hbox2, flag= wx.ALIGN_CENTER|wx.TOP|wx.BOTTOM, border=10)
				okButton.Bind(wx.EVT_BUTTON, self.OnOk)
				closeButton.Bind(wx.EVT_BUTTON, self.OnClose)
		...
		dlg = Dlg( self.parent, 'Select subtitle' )
		if dlg.ShowModal() == wx.ID_OK:
			subtitle = dlg.GetSubtitle()
			logging.debug( "You chose the following subtitle:" )
			if( subtitle is None ):
				self.SelectedSubtitlesCallback( ok = False, subtitle = None )
			else:
				self.SelectedSubtitlesCallback( ok = True, subtitle = subtitle )
		else:
			self.SelectedSubtitlesCallback( ok = False, subtitle = None )
		dlg.Destroy()
'''
