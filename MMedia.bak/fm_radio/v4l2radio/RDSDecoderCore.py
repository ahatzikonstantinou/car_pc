...
__all__ = ["RDSDecoderError", "RDSDecoderUnavailableError",
           "RDSDecoderListener", "SegmentedString", "RDSDecoderCore"]
...
class RDSDecoderCore(object):
    """
    Class for decoding RDS data from a V4L2 FM Radio
    """
    
    def __init__(self, radio, fd):
        self.__fd = fd
    ...
    def __parsing_loop(self):
        ...
        self.__current_group = {}
        #print( 'RDSDecoder is in __parsing_loop' )
        #return
        with self.radio.deviceLock:           
            #try:
                #fd = open(self.radio.dev, 'r')
            #except OSError:
                #raise RDSDecoderUnavailableError("Radio device for RDS Decoder is not available.") 
            for i in range(0, 4):
                ...
                #block = fd.read(3)
                try:
                    block = os.read( self.__fd, 3 )
                except:
                    #the device was opened non-blocking so the error most probably is "device not available"
                    break
        ...
        #print( 'RDSDecoder finished the __parsing_loop' )
        #print( 'RDSDecoder woke up in the __parsing_loop inner while and will rerun' )
        #print( 'self.__running={}'.format( self.__running ) )
