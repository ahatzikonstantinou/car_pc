...
from media_device import MediaDeviceSettings
...
class TVSettings( MediaDeviceSettings ):
	'''These are the settings of the tv'''

	Filename = 'settings.tv' #the file were the settings are saved
	...
	def __init__( self ):
		...
		self._speeddial_rows = 2
		self._speeddial_buttons_per_row = 8
	...
	def Load( self ):
		if( not os.path.isfile( TVSettings.Filename ) ):
			return
		with open( TVSettings.Filename, mode='r' ) as f:
			settingsDict = {}
			dict = json.load( f )
			#pprint.pprint( dict )
	...
	def Save(self):
		with open( TVSettings.Filename, mode='w' ) as f:
